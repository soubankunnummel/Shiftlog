...
const DEFAULT_SETTINGS = { hourlyRate: 0, idleThresholdHours: 6, reminderTimes: [] as string[], timezoneOffset: 0 };

// Mutation-side: guarantees a settings row exists, creating it on first use.
...
export const start = mutation({
  args: {},
  handler: async (ctx) => {
    const existing = await ctx.db.query('running').first();
...
export const updateSettings = mutation({
  ...
  handler: async (ctx, patch) => {
    ...
    if (patch.reminderTimes !== undefined) clean.reminderTimes = patch.reminderTimes;
